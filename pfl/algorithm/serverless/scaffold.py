...
from pfl.serverless.test import ServerlessFederatedNNAlgorithm
...
SCAFFOLDCentralContextType = CentralContext[SCAFFOLDParams, ModelHyperParamsType]


class ServerlessSCAFFOLD(
    ServerlessFederatedNNAlgorithm[
        SCAFFOLDParams, ModelHyperParamsType, StatefulModelType, MappedVectorStatistics, AbstractDatasetType
    ]
):
    """
    ...
    ) -> Tuple[Optional[Tuple[SCAFFOLDCentralContextType, ...]], StatefulModelType, Metrics]:
        if iteration == 0:
            # initial state all zeros.
            self._server_c = model.get_parameters().apply_elementwise(lambda w: w * 0)
    ...
    def process_aggregated_statistics(
        self,
        central_context: SCAFFOLDCentralContextType,
        ...
        self._server_c += MappedVectorStatistics(
            {
                k.split("/")[1]: (v * central_context.cohort_size / central_context.algorithm_params.population)
        ...
        model_update = MappedVectorStatistics(
            {k.split("/")[1]: v for k, v in statistics.items() if k.startswith("model_update/")}
        )
        new_model, metrics = model.apply_model_update(model_update)

        server_c_norm = get_ops().global_norm(self._server_c.get_weights()[1], order=2)
        metrics[StringMetricName("server_c norm")] = Weighted.from_unweighted(server_c_norm)
        return new_model, metrics

    def train_one_user(
        self,
        initial_model_state: WeightedStatistics,
        model: StatefulModel,
        ...
        assert user_dataset.user_id is not None, "SCAFFOLD requires datasets to have the `user_id` property set"
        # Local training loop
        model_params = central_context.model_train_params
        assert model_params.local_num_steps is None, "Specifying local_num_steps is not supported yet for SCAFFOLD"
        ...
        if central_context.algorithm_params.use_gradient_as_control_variate:
            bridges.sgd_bridge().do_sgd(
                model,
                user_dataset,
                model_params.static_clone(
                    local_learning_rate=1.0, local_batch_size=None, local_num_epochs=None, local_num_steps=1
                ),
            )
        ...
        bridges.scaffold_bridge().do_control_variate_sgd(
            model, user_dataset, model_params, local_c=local_c, server_c=self._server_c
        )
        ...
        model_diff = model.get_model_difference(
            initial_model_state, clone=central_context.algorithm_params.use_gradient_as_control_variate
        )
        ...
        if not central_context.algorithm_params.use_gradient_as_control_variate:
            ...
            batch_size = data_size if model_params.local_batch_size is None else model_params.local_batch_size
            K = model_params.local_num_epochs * math.ceil(data_size / batch_size)
            local_c_plus += model_diff.apply_elementwise(lambda w: w / (K * model_params.local_learning_rate))

        storage.save_state(local_c_plus, str(user_dataset.user_id), "local_c")

        # Combine model update and local_c diff into one payload.
        user_payload = MappedVectorStatistics({f"model_update/{k}": v for k, v in model_diff.items()})
