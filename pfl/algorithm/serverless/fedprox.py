...
from pfl.algorithm.serverless.federated_averaging import ServerlessFederatedAveraging
...
class AdaptMuOnMetricCallback(HyperParam[float], TrainingProcessCallback):
    ...
    def after_central_iteration(
        self, aggregate_metrics: Metrics, model: ModelType, *, central_iteration: int
    ) -> Tuple[bool, Metrics]:
        """
        ...
        if central_iteration % self.adapt_frequency == 0:
            ...
            if new_loss < self.prev_loss:
                ...
                if self.loss_decrease >= self.decrease_mu_after_consecutive_improvements:
...
class ServerlessFedProx(ServerlessFederatedAveraging):
    """
    ...
    def train_one_user(
        ...
        bridges.fedprox_bridge().do_proximal_sgd(
            model, user_dataset, central_context.model_train_params, mu=central_context.algorithm_params.mu
        )
